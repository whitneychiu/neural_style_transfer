...
from helpers.style_utils import features_from_img, extract_features, rel_error, style_transfer
from helpers.image_utils import preprocess
...
cnn = torchvision.models.squeezenet1_1(weights='SqueezeNet1_1_Weights.DEFAULT').to(device).features
...
pd = {1: params1, 2: params2, 3: params3}
for i in tqdm(range(len(pd)), desc="Transferring content/style...", leave=True):
    style_transfer(**pd[i + 1])
